    ...
    <div className="min-h-screen bg-[#0A0E1A] pb-12">
      ...
      <div className="px-8 py-8 max-w-2xl space-y-6">
        ...
        <form onSubmit={async (e) => {
          ...
          const key1 = (form.querySelector('[name="key1"]') as HTMLInputElement)?.value;
          const key2 = (form.querySelector('[name="key2"]') as HTMLInputElement)?.value;
          const res = await fetch('/api/ai/settings', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('deployflow_token')}` },
            body: JSON.stringify({ key1, key2 })
          });
          const d = await res.json();
          setStatus({ type: d.ok ? 'success' : 'error', msg: d.ok ? '✅ Groq API keys disimpan!' : '❌ ' + d.error });
        }} className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-5">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            🤖 Groq AI API Keys
          </h2>
          <p className="text-xs text-gray-500">Dual API key — kalau key 1 limit, otomatis pindah ke key 2. Buat di <a href="https://console.groq.com" target="_blank" className="text-indigo-400">console.groq.com</a></p>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">API Key 1 (Primary)</label>
            <input name="key1" type="password" placeholder="gsk_..." className="w-full px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500/50 text-sm font-mono"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">API Key 2 (Fallback)</label>
            <input name="key2" type="password" placeholder="gsk_..." className="w-full px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500/50 text-sm font-mono"/>
          </div>
          <button type="submit" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 text-sm font-semibold text-white hover:opacity-90">
            💾 Simpan API Keys
          </button>
        </form>
        {/* Setup Guide */}
