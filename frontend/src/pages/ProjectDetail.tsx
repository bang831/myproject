...
import {
  ArrowLeft, Bot, FolderOpen, ExternalLink, GitBranch, Clock, CheckCircle2, XCircle,
...
// ─── Variables Panel ──────────────────────────────────────────────────────
function VariablesPanel({ projectId }: { projectId: string }) {
  ...
  const [showVals, setShowVals] = React.useState<Record<string,boolean>>({});
  const token = () => localStorage.getItem("deployflow_token");

  React.useEffect(() => {
    fetch(`/api/projects/${projectId}/env`, { headers: { Authorization: `Bearer ${token()}` } })
      .then(r => r.json()).then(d => setVars(d.env_vars || {})).catch(() => {});
  }, [projectId]);
  ...
  const save = async (updated: Record<string,string>) => {
    ...
    try {
      ...
      setMsg(d.ok ? "✅ Disimpan & .env diupdate!" : "❌ " + d.error);
    } catch { setMsg("❌ Gagal"); } finally { setSaving(false); }
  };

  const addVar = () => {
    if (!newKey.trim()) return;
    const updated = { ...vars, [newKey.trim()]: newVal };
    setVars(updated); setNewKey(""); setNewVal("");
    save(updated);
  };
  ...
  const removeVar = (key: string) => {
    const updated = { ...vars };
    delete updated[key];
    setVars(updated); save(updated);
  };

  const updateVal = (key: string, val: string) => {
    const updated = { ...vars, [key]: val };
    setVars(updated);
  };
  ...
  const importEnv = async () => {
    setSaving(true); setMsg("");
    try {
      const res = await fetch(`/api/projects/${projectId}/env/import`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
        body: JSON.stringify({ content: envText })
      });
      const d = await res.json();
      if (d.ok) { setVars(d.vars); setEnvText(""); setImporting(false); setMsg(`✅ ${d.count} variabel diimport!`); }
      else setMsg("❌ " + d.error);
    } catch { setMsg("❌ Gagal"); } finally { setSaving(false); }
  };
      ...
      <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => { setImporting(!importing); setMsg(""); }}
            ...
            <button onClick={() => save(vars)} disabled={saving}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-500/20 border border-indigo-500/30 text-xs text-indigo-300 hover:bg-indigo-500/30 disabled:opacity-40">
              {saving ? "..." : "💾 Save All"}
            </button>
          </div>
        </div>
        ...
        {importing && (
          <div className="mb-4 space-y-2">
            <p className="text-xs text-gray-500">Paste isi file .env kamu:</p>
            <textarea value={envText} onChange={e => setEnvText(e.target.value)}
              rows={6} placeholder="DB_HOST=localhost&#10;DB_PORT=5432&#10;SECRET_KEY=xxx"
              className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white text-xs font-mono focus:outline-none focus:border-indigo-500/50 resize-none" />
            <div className="flex gap-2">
              <button onClick={importEnv} disabled={saving || !envText}
                className="px-4 py-2 rounded-xl bg-indigo-500/20 border border-indigo-500/30 text-sm text-indigo-300 hover:bg-indigo-500/30 disabled:opacity-40">
                {saving ? "..." : "Import"}
              </button>
              <button onClick={() => { setImporting(false); setEnvText(""); }}
                className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-gray-400 hover:bg-white/10">
                Batal
              </button>
            </div>
          </div>
        ...
        {Object.keys(vars).length === 0 && !importing ? (
          <p className="text-sm text-gray-600 text-center py-6">Belum ada variabel. Tambah manual atau import .env</p>
        ) : (
          <div className="space-y-2 mb-4">
            {Object.entries(vars).map(([k, v]) => (
              <div key={k} className="flex items-center gap-2">
                <input value={k} readOnly
                  className="w-36 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-xs font-mono focus:outline-none" />
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    onBlur={() => save(vars)}
                    className="w-full px-3 py-2 pr-8 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-xs font-mono focus:outline-none focus:border-indigo-500/50" />
                  <button type="button"
                ...
                <button onClick={() => removeVar(k)}
                  className="p-2 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-500/5 transition-all text-xs">
                  ✕
                </button>
              </div>
            ))}
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-white/5">
          <input value={newKey} onChange={e => setNewKey(e.target.value.toUpperCase().replace(/ /g,"_"))}
            placeholder="VARIABLE_NAME"
            onKeyDown={e => e.key === "Enter" ? addVar() : null}
            className="w-36 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-xs font-mono focus:outline-none focus:border-indigo-500/50" />
          <span className="text-gray-600">=</span>
          <input value={newVal} onChange={e => setNewVal(e.target.value)}
            placeholder="value"
            onKeyDown={e => e.key === "Enter" ? addVar() : null}
            className="flex-1 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-xs font-mono focus:outline-none focus:border-indigo-500/50" />
          <button onClick={addVar} disabled={!newKey}
            className="px-4 py-2 rounded-xl bg-green-500/20 border border-green-500/30 text-xs text-green-300 hover:bg-green-500/30 disabled:opacity-40">
            + Add
          </button>
        </div>

        {msg && <p className="text-xs mt-3 text-gray-400 bg-white/[0.02] px-3 py-2 rounded-lg">{msg}</p>}
      </div>
    </div>
  );
}

function ProjectConfigPanel({ projectId, projectType }: { projectId: string; projectType: string }) {
  const [tab, setTab]             = React.useState<"path"|"backend">("path");
  const [files, setFiles]         = React.useState<string[]>([]);
  const [rootPath, setRootPath]   = React.useState("");
  const [indexFile, setIndexFile] = React.useState("");
  const [startCmd, setStartCmd]   = React.useState("");
  const [backendPath, setBackendPath] = React.useState("");
  const [saving, setSaving]       = React.useState(false);
  const [msg, setMsg]             = React.useState("");
  const token = () => localStorage.getItem("deployflow_token");
  ...
  const setPath = async () => {
    setSaving(true); setMsg("");
    try {
      const res = await fetch(`/api/projects/${projectId}/set-path`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
        body: JSON.stringify({ root_path: rootPath, index_file: indexFile || "index.html" })
      });
      const d = await res.json();
  ...
  const setIndex = async (file: string) => {
    setSaving(true); setMsg("");
    try {
      const res = await fetch(`/api/projects/${projectId}/set-index`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
        body: JSON.stringify({ index_file: file })
      });
      const d = await res.json();
      setMsg(d.ok ? "✅ " + d.message : "❌ " + d.error);
    } catch { setMsg("❌ Gagal"); } finally { setSaving(false); }
  ...
  const startBackend = async () => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/set-path`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
        body: JSON.stringify({ start_command: startCmd, backend_path: backendPath })
      });
      const d = await res.json();
      setMsg(d.ok ? "✅ " + d.message : "❌ " + d.error);
    } catch { setMsg("❌ Gagal"); } finally { setSaving(false); }
  ...
  return (
    <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-4">
      <h3 className="text-sm font-semibold text-white">⚙️ Konfigurasi Project</h3>
      <div className="flex gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/[0.06]">
        {[{id:"path",label:"📁 Static / Frontend"},{id:"backend",label:"🚀 Backend / Node.js"}].map(t => (
          <button key={t.id} type="button" onClick={() => { setTab(t.id as any); setMsg(""); }}
      ...
      {tab === "path" && (
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Root Path <span className="text-gray-600">(folder utama, misal: frontend, dist, public)</span></label>
            <div className="flex gap-2">
              <input value={rootPath} onChange={e => setRootPath(e.target.value)} placeholder="frontend"
                className="flex-1 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-sm font-mono focus:outline-none focus:border-indigo-500/50" />
              <input value={indexFile} onChange={e => setIndexFile(e.target.value)} placeholder="index.html"
                className="w-32 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-sm font-mono focus:outline-none focus:border-indigo-500/50" />
              <button onClick={setPath} disabled={saving}
                className="px-4 py-2 rounded-xl bg-indigo-500/20 border border-indigo-500/30 text-sm text-indigo-300 hover:bg-indigo-500/30 disabled:opacity-40">
                {saving ? "..." : "Set"}
              </button>
            </div>
            <p className="text-xs text-gray-600 mt-1.5">Format: folder/subfolder atau kosongkan jika di root</p>
          </div>
          {files.length > 0 && (
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">Atau klik file HTML untuk langsung set sebagai homepage:</label>
              <div className="grid grid-cols-2 gap-1.5 max-h-40 overflow-y-auto">
                {files.map(f => (
                  <button key={f} onClick={() => setIndex(f)} disabled={saving}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/[0.06] text-xs text-gray-400 hover:bg-indigo-500/10 hover:text-indigo-300 hover:border-indigo-500/30 transition-all text-left disabled:opacity-40">
                    📄 {f}
                  </button>
                ))}
              </div>
      ...
      {tab === "backend" && (
        <div className="space-y-3">
          <p className="text-xs text-gray-500">Jalankan backend Node.js/Express via PM2.</p>
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Folder Backend <span className="text-gray-600">(kosongkan jika di root, misal: backend, server, api)</span></label>
            <input value={backendPath} onChange={e => setBackendPath(e.target.value)} placeholder="backend"
              className="w-full px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-sm font-mono focus:outline-none focus:border-indigo-500/50 mb-3" />
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Start Command</label>
            <div className="flex gap-2">
              <input value={startCmd} onChange={e => setStartCmd(e.target.value)} placeholder="node server.js"
                className="flex-1 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-sm font-mono focus:outline-none focus:border-indigo-500/50" />
              <button onClick={startBackend} disabled={saving || !startCmd}
                className="px-4 py-2 rounded-xl bg-green-500/20 border border-green-500/30 text-sm text-green-300 hover:bg-green-500/30 disabled:opacity-40">
                {saving ? "..." : "▶ Run"}
              </button>
            </div>
          </div>
          <div className="text-xs text-gray-600 bg-white/[0.02] rounded-xl p-3 space-y-1">
            <p className="text-gray-500 mb-1">Contoh konfigurasi:</p>
            <p>Folder: <span className="font-mono text-gray-400">backend</span> → Command: <span className="font-mono text-gray-400">node server.js</span></p>
            <p>Folder: <span className="font-mono text-gray-400">(kosong)</span> → Command: <span className="font-mono text-gray-400">npm start</span></p>
            <p>Folder: <span className="font-mono text-gray-400">api</span> → Command: <span className="font-mono text-gray-400">node index.js</span></p>
          </div>
        </div>
      )}
      {msg && <p className="text-xs mt-1 text-gray-400 bg-white/[0.02] px-3 py-2 rounded-lg">{msg}</p>}
    </div>
  );
}
export default function ProjectDetail({ user }: { user: any }) {
  ...
  const [activeTab, setActiveTab]     = useState<'overview' | 'deployments' | 'logs' | 'variables' | 'files' | 'ai'>('overview');
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A]">
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {activeTab === 'overview' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <ProjectConfigPanel projectId={id!} projectType={project.project_type || "static"} />
        ...
        {activeTab === 'variables' && (
          <VariablesPanel projectId={id!} />
        )}
        {activeTab === 'files' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <FileManager projectId={id!} appId={project.app_id} />
          </motion.div>
        )}

        {activeTab === 'ai' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <AIAssistant projectId={id!} appId={project.app_id} projectName={project.name} />
          </motion.div>
        )}
